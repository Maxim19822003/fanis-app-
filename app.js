...
const App = {
    ...
    goTo(screenId) {
        // Сброс isAdmin при уходе из админки в обычный экран
        const currentScreen = this.history.length > 0 ? this.history[this.history.length - 1] : null;
        if (currentScreen && currentScreen.startsWith('admin-') && !screenId.startsWith('admin-')) {
            this.isAdmin = false;
            this.currentAdminErrorSystem = null;
        }
    ...
    openError(id) {
        ...
        let html = '';
        let imgUrl = err.image_url || '';
        if (imgUrl && imgUrl !== '{}' && !imgUrl.startsWith('http')) {
            imgUrl = 'https://' + imgUrl;
        }
        if (imgUrl && imgUrl !== '{}' && imgUrl.startsWith('http')) {
            html += `<div class="card-mem"><img src="${imgUrl}" alt="мем" loading="lazy" onerror="this.style.display='none';this.parentElement.style.display='none'"></div>`;
        }
        html += `
    ...
    // ========== АДМИН: ОШИБКИ (по системам + сортировка) ==========
    async renderAdminErrors() {
        const list = document.getElementById('admin-err-list');
        const titleEl = document.getElementById('admin-err-title');
        
        if (!this.currentAdminErrorSystem) {
            if (titleEl) titleEl.textContent = 'Коды ошибок — Выбор системы';
            list.innerHTML = `
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
                    <button class="action-btn" onclick="app.selectAdminErrorSystem('UMAR')">UMAR</button>
                    <button class="action-btn" onclick="app.selectAdminErrorSystem('CLE')">CLE</button>
                    <button class="action-btn" onclick="app.selectAdminErrorSystem('UMAR_new')">UMAR_new</button>
                    <button class="action-btn" onclick="app.selectAdminErrorSystem('CLE_new')">CLE_new</button>
                </div>
            `;
            return;
        }

        // Загружаем ошибки выбранной системы
        try {
            const response = await fetch(`${API_URL}/api/errors/${this.currentAdminErrorSystem}`);
            if (response.ok) {
                const fetchedErrors = await response.json();
                // Обновляем кэш: удаляем старые ошибки этой системы, добавляем новые
                this.data.errors = this.data.errors.filter(e => e.system !== this.currentAdminErrorSystem).concat(fetchedErrors);
            }
        } catch (e) {}

        const systemErrors = this.data.errors
            .filter(e => e.system === this.currentAdminErrorSystem)
            .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

        if (titleEl) titleEl.textContent = this.currentAdminErrorSystem + ' — Коды ошибок';

        let html = `<button class="action-btn btn-back" onclick="app.clearAdminErrorSystem()" style="margin-bottom:12px;">⬅️ Назад к системам</button>`;

        if (systemErrors.length === 0) {
            html += '<p style="color:var(--text-secondary);text-align:center;padding:20px;">Нет ошибок. Нажмите ➕</p>';
        } else {
            html += systemErrors.map((e, index) => `
                <div class="admin-list-item" style="display:flex;align-items:center;gap:8px;">
                    <div style="display:flex;flex-direction:column;gap:2px;">
                        <button onclick="app.moveError(${e.id}, -1)" style="background:var(--bg-input);border:none;color:var(--text-secondary);padding:2px 6px;border-radius:4px;font-size:12px;cursor:pointer;">↑</button>
                        <button onclick="app.moveError(${e.id}, 1)" style="background:var(--bg-input);border:none;color:var(--text-secondary);padding:2px 6px;border-radius:4px;font-size:12px;cursor:pointer;">↓</button>
                    </div>
                    <div style="flex:1;" onclick="app.editError(${e.id})">
                        <div class="item-info">
                            <div class="item-name">${e.code} · ${e.description || ''}</div>
                            <div class="item-meta">${e.period || ''} ${e.image_url ? '🖼️' : ''}</div>
                        </div>
                    </div>
    ...
    selectAdminErrorSystem(system) {
        this.currentAdminErrorSystem = system;
        this.renderAdminErrors();
    },

    clearAdminErrorSystem() {
        this.currentAdminErrorSystem = null;
        this.renderAdminErrors();
    },

    async moveError(id, direction) {
        const systemErrors = this.data.errors
            .filter(e => e.system === this.currentAdminErrorSystem)
            .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
        
        const idx = systemErrors.findIndex(e => e.id === id);
        if (idx === -1) return;
        
        const newIdx = idx + direction;
        if (newIdx < 0 || newIdx >= systemErrors.length) return;
        
        const temp = systemErrors[idx];
        systemErrors[idx] = systemErrors[newIdx];
        systemErrors[newIdx] = temp;
        
        const order = systemErrors.map((e, i) => ({ id: e.id, sort_order: i }));
        
        try {
            await fetch(`${API_URL}/api/errors/reorder`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ order })
            });
        ...
        this.data.errors = this.data.errors.filter(e => e.system !== this.currentAdminErrorSystem).concat(systemErrors);
        this.renderAdminErrors();
    },
    ...
    async saveError() {
        ...
        try {
            ...
            if (response.ok) {
                this.showToast('✅ Сохранено');
                this.currentAdminErrorSystem = data.system;
                await this.loadData();
    ...
    async deleteError() {
        if (!this.editingId || !confirm('Удалить?')) return;
        const err = this.data.errors.find(e => e.id === this.editingId);
        try {
            await fetch(`${API_URL}/api/errors/${this.editingId}`, { method: 'DELETE' });
            this.showToast('🗑️ Удалено');
            if (err) this.currentAdminErrorSystem = err.system;
            await this.loadData();
            this.goTo('admin-errors');
        } catch (e) {
